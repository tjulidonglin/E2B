    ...
    def _calculate_statistics(self, results: List[Tuple], mode: str, failed: int = 0) -> Dict[str, Any]:
        ...
        def calc_stats(times: List[float]) -> Dict:
            ...
            return {
                ...
                'p95': sorted_times[int(n * 0.95)] if n >= 2 else times[0],
            }
        ...
        print(f"\n  {'Metric':<12} {'Min':>10} {'Max':>10} {'Mean':>10} {'P95':>10}")
        for label, stats in [("Create", create_stats), ("Probe", probe_stats), ("Total", total_stats)]:
            if stats:
                print(f"  {label:<12} {stats['min']:>10.3f} {stats['max']:>10.3f} {stats['mean']:>10.3f} {stats['p95']:>10.3f}")
        ...
        self.report.add_test_result(
            ...
            metrics={'Avg Total': f"{total_stats['mean']:.3f}s", 'P95': f"{total_stats['p95']:.3f}s"}
        )
